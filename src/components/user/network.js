...
router.post('/', async (req, res) => {
    ...
    if (!secret) {
        ...
        await controller.addUser(req.body).then(resolve => {
...
router.get('/', async (req, res) => {
    ...
    if (!secret) {
        ...
        await controller.showAll().then(resolve => {
            bodyRes.error = '';
...
router.get('/byid', async (req, res) => {
    ...
    if (!secret) {
        ...
        await controller.getUserById(uid).then(resolve => {
            bodyRes.error = '';
            bodyRes.message = "Success query";
            bodyRes.data = resolve;
            response.success(res, bodyRes);
...
router.put('/', async (req, res) => {
    ...
    if (!secret) {
        ...
        await controller.updateUser(req.body).then(resolve => {
            bodyRes.message = resolve;
            bodyRes.data = {};
            bodyRes.error = '';
